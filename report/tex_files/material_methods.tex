...
\subsection{4.1 Materiales}

El análisis se basó en información procedente de bases de datos biomédicas de referencia.
Se seleccionó el término \textbf{HP:0002344 – Progressive neurologic deterioration} de la Human Phenotype Ontology (HPO) \cite{HPO_term_HP0002344,HPO2025} como punto de partida, recopilando los genes asociados a este fenotipo.
La relación gen–enfermedad se contrastó con OMIM \cite{Amberger2011} y GeneCards \cite{stelzer2016genecards}, lo que permitió confirmar y ampliar la evidencia disponible sobre cada gen.

Las interacciones entre las proteínas codificadas por dichos genes se obtuvieron mediante STRINGdb (versión 12.0) \cite{snel2000string}, considerando únicamente interacciones de alta confianza (combined score $\geq$ 0.7) en \textit{Homo sapiens}.

El procesamiento y análisis de redes se realizaron en R (versión 4.3.2), empleando los paquetes \texttt{igraph}, \texttt{tidygraph}, \texttt{ggraph} y \texttt{linkcomm} \cite{igraph2006,tidygraph2020,ggraph2017,linkcomm2012}.
...
\subsection{4.2 Metodología}

\subsubsection{4.2.0 Flujo de trabajo general}
...
\subsubsection{4.2.1 Recopilación de genes y construcción de la red}

A partir del término HP:0002344 se descargó la lista de genes relacionados desde HPO \cite{HPO_term_HP0002344}.
Tras eliminar duplicados, se verificó la validez de las asociaciones gen–enfermedad mediante OMIM \cite{Amberger2011} y GeneCards \cite{stelzer2016genecards}, obteniendo un conjunto depurado de genes candidatos.

Estos genes se introdujeron en STRINGdb (v12.0) \cite{snel2000string} para obtener la red de interacciones proteína–proteína (PPI), restringiéndonos a interacciones con evidencia experimental o provenientes de bases de datos curadas.
La red se exportó y procesó en R, transformándose en un objeto \texttt{igraph} para su análisis estructural.

\subsubsection{4.2.2 Análisis estructural de la red}
...
\subsubsection{4.2.3 Identificación de comunidades funcionales}

La detección de comunidades se realizó mediante dos algoritmos ampliamente utilizados: Louvain \cite{blondel2008louvain} y Walktrap \cite{pons2005walktrap}, implementados en \texttt{igraph}.
Además, se aplicó el enfoque de comunidades solapadas mediante el paquete \texttt{linkcomm} \cite{linkcomm2012}, permitiendo que un mismo gen pudiera participar en distintos módulos cuando intervenía en procesos independientes.
...
\subsubsection{4.2.4 Análisis de genes de interés}
...
\subsubsection{4.2.5 Enriquecimiento funcional}
